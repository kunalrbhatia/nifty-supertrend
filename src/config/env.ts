...
const envSchema = z
  ...
  .refine(
    (data) => {
      ...
      if (
        isSlackActuallyEnabled &&
        (!data.SLACK_WEBHOOK_URL || data.SLACK_WEBHOOK_URL.trim() === '')
      ) {
        return false;
      }
      return true;
    ...
    {
      message:
        'SLACK_WEBHOOK_URL is required when SLACK_ENABLED is true and TELEGRAM_ENABLED is false',
