...
describe('LogsCommand', () => {
  ...
  it('should return PM2 logs if available', async () => {
    jest.spyOn(child_process, 'execSync').mockReturnValue('pm2 logs output' as any);
    await logsHandler(mockCtx);
    expect(mockCtx.reply).toHaveBeenCalledWith(expect.stringContaining('pm2 logs output'), expect.any(Object));
  });
  ...
  it('should fallback to file logs if PM2 fails', async () => {
    ...
    await logsHandler(mockCtx);
    expect(mockCtx.reply).toHaveBeenCalledWith(expect.stringContaining('file log output'), expect.any(Object));
  });

  it('should handle errors gracefully', async () => {
    jest.spyOn(child_process, 'execSync').mockImplementation(() => { throw new Error('fail'); });
    jest.spyOn(fs, 'readdirSync').mockImplementation(() => { throw new Error('FS error'); });
